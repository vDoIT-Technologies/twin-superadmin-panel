...
function StatusBadge({ status, isVault }) {
  const badgeTone = status === 'Configured'
    ? (isVault ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-indigo-200 bg-indigo-50 text-indigo-700')
    : (statusTone[status] || statusTone.Unknown);
  const dotTone = status === 'Configured'
    ? (isVault ? 'bg-emerald-500' : 'bg-indigo-500')
    : (statusDotTone[status] || statusDotTone.Unknown);

  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-semibold ${badgeTone}`}>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-panel sm:p-6">
        {isLoading ? (
          <div className="flex min-h-48 items-center justify-center gap-3 text-sm font-medium text-slate-500">
            <span className={`h-5 w-5 animate-spin rounded-full border-2 ${isVault ? 'border-emerald-200 border-t-emerald-600' : 'border-indigo-200 border-t-indigo-600'}`} aria-hidden="true" />
